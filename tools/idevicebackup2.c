...
// Updated protocol for new IOS devices to the latest version 17.5.1
// Added reconnection to apple servers, which makes it possible to reconnect in case of constant access to servers and fixes timeout issue.
// The remove_directory and remove_file functions for Windows OS have been updated for all possible paths, as well as for network paths.
...
static void mb2_handle_send_files(mobilebackup2_client_t mobilebackup2, plist_t message, const char* backup_dir)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		//printf("[mb2_handle_send_files]: backup_dir[%s] str[%s]\n", backup_dir, str);
		if (!str)
			continue;
...
static int mb2_handle_receive_files(mobilebackup2_client_t mobilebackup2, plist_t message, const char* backup_dir)
{
	...
	do {
		...
		//printf("[mb2_handle_receive_files]: backup_dir[%s] dname[%s] fname[%s] bname[%s]\n", backup_dir, dname, fname, bname);
		f = _wfopen(file, L"wb");
...
static void mb2_handle_list_directory(mobilebackup2_client_t mobilebackup2, plist_t message, const char* backup_dir)
{
	...
	//wprintf(L"[mb2_handle_list_directory] %ls\n", path);
	...
	if (cur_dir) {
		...
		while ((ep = _wreaddir(cur_dir))) {
			...
			if (fpath) {
				...
				//printf("[mb2_handle_list_directory]: key [%s] ftype[%s]\n", key, ftype);
...
#define OPT_FULL 9
	...
	if (cmd == CMD_CHANGEPW) {
		...
		if (cmd == CMD_RESTORE || cmd == CMD_UNBACK) {
			...
			plist_read_from_filename(manifest_path, &manifest_plist, NULL);
			if (!manifest_plist) {
				idevice_free(device);
				free(info_path);
				free(manifest_path);
...
checkpointRestart:
	...
	if ((ldret == LOCKDOWN_E_SUCCESS) && service && service->port) {
		...
		if (err != MOBILEBACKUP2_E_SUCCESS) {
			if (reconnect < 30) {
				printf("Could not perform backup protocol version exchange => reconnect: %d\n", reconnect);
				reconnect++;
				goto checkpointRestart;
			}
	...
	checkpoint:
		...
		case CMD_BACKUP:
			...
			__wmkdir(AnsiToWideChar(devbackupdir), 0755);
			free(devbackupdir);
							...
							if (key && (plist_get_node_type(val) == PLIST_STRING)) {
								...
								if (str) {
									...
									if ((_wstat(AnsiToWideChar(newpath), &st) == 0) && S_ISDIR(st.st_mode)) {
										int statusR = rmdir_recursive(newpath);
									}
									else {
										int statusR = remove_file(newpath);
									}
										
									wchar_t* oldpath_w = AnsiToWideChar(oldpath);
									wchar_t* newpath_w = AnsiToWideChar(newpath);
									...
									if (_wrename(AnsiToWideChar(oldpath), AnsiToWideChar(newpath)) != 0) {
										...
										printf("[MOVING]: str[%s] key[%s]\n", str, key);
										printf("[MOVING2]: newpath[%s] oldpath[%s]\n", newpath, oldpath);
										break;
									}
									free(oldpath);
									free(newpath);
